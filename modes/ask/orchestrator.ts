...
function normalizeWorkspaceRelPath(input: string): string {
  const raw = (input ?? "").trim();
  if (!raw) return raw;

  // block traversal / home expansion / windows paths
  if (raw.includes("..")) throw new Error("Invalid path: '..' is not allowed");
  if (raw.startsWith("~")) throw new Error("Invalid path: '~' is not allowed");
  if (raw.includes("\\")) throw new Error("Invalid path: backslashes are not allowed");

  // treat "/foo/bar" and "./foo/bar" as "foo/bar" (workspace-relative)
  return raw.replace(/^\.?\//, "");
}
...
function createAskTools(executor: ToolExecutor) {
  return {
    read_file: tool({
      description:
        "Read a text file from the workspace. Use a path relative to the project root. Leading '/' is treated as workspace-relative.",
      inputSchema: z.object({
        path: z.string().describe("Relative file path"),
      }),
      execute: async ({ path: p }) =>
        executor.readFile(normalizeWorkspaceRelPath(p)),
    }),

    list_files: tool({
      description:
        "List files and directories under a path. Leading '/' is treated as workspace-relative.",
      inputSchema: z.object({
        path: z.string(),
        recursive: z.boolean().optional().default(false),
      }),
      execute: async ({ path: p, recursive }) =>
        executor.listFiles(normalizeWorkspaceRelPath(p), recursive),
    }),
    ...
    search_files: tool({
      ...
      execute: async ({ root, pattern, content_contains }) =>
        executor.searchFiles(
          normalizeWorkspaceRelPath(root),
          pattern,
          content_contains
        ),
    }),
    ...
    analyze_codebase: tool({
      ...
      execute: async ({ path: p }) =>
        executor.analyzeCodebase(normalizeWorkspaceRelPath(p)),
    }),
